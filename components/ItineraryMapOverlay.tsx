...
const ItineraryMapOverlay: React.FC<ItineraryMapOverlayProps> = ({ city, onClose }) => {
  ...
  // Delegated click handler for Leaflet popup buttons (avoids window globals).
  // Leaflet renders popups in a separate pane outside the map container div,
  // so we attach to document to catch bubbled clicks from popup buttons.
  const savedPOIsRef = useRef(savedPOIs);
  savedPOIsRef.current = savedPOIs;

  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      const btn = (e.target as HTMLElement).closest('[data-poi-action]') as HTMLElement | null;
      if (!btn) return;
      const action = btn.dataset.poiAction;
      const poiId = btn.dataset.poiId;
      if (!poiId) return;
      if (action === 'edit') {
        const poi = savedPOIsRef.current.find(p => p.id === poiId);
        if (poi) {
          setEditingPOI(poi);
